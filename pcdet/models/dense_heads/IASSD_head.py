...
class IASSD_Head(PointHeadTemplate):
    ...
    def get_center_cls_layer_loss(self, tb_dict=None):
        point_cls_labels = self.forward_ret_dict['center_cls_labels'].view(-1) # torch.Size([2048]) 
        point_cls_preds = self.forward_ret_dict['center_cls_preds'].view(-1, self.num_class) # torch.Size([2048, 3]) 
    ...
    def get_sa_ins_layer_loss(self, tb_dict=None):
        ...
        for i in range(len(sa_ins_labels)): # valid when i =1, 2
            if len(sa_ins_preds[i]) != 0:
                ...
                ignore += 1
                continue
            point_cls_labels = sa_ins_labels[i].view(-1) # torch.Size([8192]) torch.Size([4096])
            # print("!!!!!!!!!!!!!!!!!")
            # print("point_cls_labels",point_cls_labels)
            # print("point_cls_preds",point_cls_preds)
            # 前景点
            positives = (point_cls_labels > 0)
            # 背景点赋予权重
            negative_cls_weights = (point_cls_labels == 0) * 1.0
            ...
            pos_normalizer = positives.sum(dim=0).float() # tensor(1873.)
            # 正则化分类损失,根据论文中所述，用正样本数量来正则化分类损失
            cls_weights /= torch.clamp(pos_normalizer, min=1.0)

            one_hot_targets = point_cls_preds.new_zeros(*list(point_cls_labels.shape), self.num_class + 1)
            # print("111111111")
            # print(one_hot_targets.shape)
            # print((point_cls_labels * (point_cls_labels >= 0).long()).unsqueeze(dim=-1).long().shape)
            one_hot_targets.scatter_(-1, (point_cls_labels * (point_cls_labels >= 0).long()).unsqueeze(dim=-1).long(), 1.0)
            one_hot_targets = one_hot_targets[..., 1:]
